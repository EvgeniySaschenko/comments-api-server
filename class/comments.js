...
class Comments {
  ...
  async addComment({ userId = 0, parentId = 0, text = '', files = []}) {
    ...
    let smtp = db.prepare("INSERT INTO comment (userId, parentId, text, files, dateCreate) VALUES (?,?,?,?,?)");
  ...
  async deleteComment(commentId) {
    let smtp = db.prepare("DELETE FROM comment WHERE id = ?");
  ...
  async updateCommentVote({ commentId, like, dislike }) {
    let smtp = db.prepare("UPDATE comment SET like = ?, dislike = ? WHERE id = ?");
  ...
  async getCommentsQuantityByParentId(parentId) {
    let stmt = db.prepare("SELECT COUNT(id) as quantity FROM comment WHERE parentId = ?");
    let result = await new Promise((resolve, reject) => {
  ...
  async getCommentById(commentId) {
    let stmt = db.prepare("SELECT * FROM comment WHERE id = ?");
    let result = await new Promise((resolve, reject) => {
  ...
  async getVoteUser(commentId, userId) {
    let stmt = db.prepare("SELECT * FROM comment_vote WHERE commentId = ? AND userId = ?");
    let result = await new Promise((resolve, reject) => {
  ...
  async addVote({commentId, userId, voteValue}) {
    ...
    let smtp = db.prepare("INSERT INTO comment_vote (userId, commentId, voteValue, dateCreate) VALUES (?,?,?,?)");
    let result = await new Promise((resolve, reject) => {
  ...
  async deleteVote(voteId) {
    let smtp = db.prepare("DELETE FROM comment_vote WHERE id = ?");
    let result = await new Promise((resolve, reject)=>{
  ...
  async updateVote({voteId, voteValue}) {
    let curentDate = parseInt(Date.now() / 1000);
    let smtp = db.prepare("UPDATE comment_vote SET voteValue = ?, dateUpdate = ? WHERE id = ?");
    let result = await new Promise((resolve, reject) => {
